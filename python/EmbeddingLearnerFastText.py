...
if __name__ == '__main__':
    ...
    model = FastText(token_seqs, min_count=5, word_ngrams=1, window=nb_tokens_in_context/2, size=embedding_size, workers=40)
    # model = Word2Vec(token_seqs, min_count=1, window=nb_tokens_in_context/2, size=embedding_size, workers=40)

    # store the model
    # time_stamp = math.floor(time.time() * 1000)
    model.save("fastText/" + "embedding_model_" + 'training')
    ...
    token_to_vector_file_name = "fastText/" + "token_to_vector_" + 'training' + ".json"
